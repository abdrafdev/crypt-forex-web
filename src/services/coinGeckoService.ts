// API service to handle CoinGecko data fetching
...
export async function fetchTopCryptos(
  ...
  try {
    ...
    const transformedData = combinedData.map((coin: any) => ({
      id: coin.id,
      ...
      size: determineSize(coin.market_cap, data[0].market_cap),
    }));
